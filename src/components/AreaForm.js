...
const AreaForm = () => {
  ...
  const areaHandler = (selectedOption) => {
    ...
    setFormData({ ...formData, areaname: selectedOption ? selectedOption.value : '' });
  ...
  const shopHandler = (selectedOption) => {
    ...
    setFormData({ ...formData, shopname: selectedOption ? selectedOption.value : '' });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5002/areas', data, {
        headers: {
  ...
  return (
    <div className='container'>
      ...
      <form className='form' onSubmit={handleSubmit}>
        <h1 style={{ color: 'red', fontSize: '25px', fontWeight: 'bold' }}>For Adding new area and shop:</h1>
        <div>
          <label className='label'>Area Name:</label>
          <input className='input' type="text" name="areaname" value={formData.areaname} onChange={handleChange} required />
        </div>
        <div>
        ...
        <div>
          ...
          <input className='input' type="text" name="shopname" value={formData.shopname} onChange={handleChange} required />
        </div>
        {formData.products.map((product, index) => (
          <div key={index}>
            <div>
              <label className='label'>Product Name:</label>
              <input className='input' type="text" name="productname" value={product.productname} onChange={(e) => handleProductChange(index, e)} required />
            </div>
            <div>
              <label className='label'>Cost:</label>
              <input className='input' type="number" name="cost" value={product.cost} onChange={(e) => handleProductChange(index, e)} required />
            </div>
            <div>
              <label className='label'>Description:</label>
              <input className='input' type="text" name="description" value={product.description} onChange={(e) => handleProductChange(index, e)} required />
            </div>
            <div>
              <label className='label'>Image:</label>
              <input className='inputimage' type="file" name="image" onChange={(e) => handleImageChange(index, e)} required />
            </div>
          </div>
        ))}
